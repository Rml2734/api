...
// =========================================================================
// RUTA ANTIGUA: RESTABLECER CLAVE (NO USADA CON EL NUEVO FLUJO JWT/SENDGRID)
// *Hemos dejado este POST por compatibilidad, pero la lógica nueva es la siguiente.*
// =========================================================================
router.post("/restablecer-clave", async (req, res) => {
    const { email, codigo, nuevaClave } = req.body;
...
// =========================================================================
// RUTA NUEVA: CONFIRMAR RESTABLECIMIENTO CON TOKEN JWT
// Recibe el token y la nueva clave y actualiza el hash en DB.
// =========================================================================
router.post("/reset-password-confirm", async (req, res) => {
    const { token, nuevaClave } = req.body;

    if (!token || !nuevaClave) {
        return res.status(400).json({ error: "Faltan datos requeridos (token y nueva clave)." });
    }

    try {
        // 1. Verificar y decodificar el token JWT
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        const { id, usuario: email } = decoded;

        if (!id) {
            return res.status(400).json({ error: "Token de recuperación inválido o incompleto." });
        }

        // 2. Hashear la nueva clave
        const nuevoHash = await bcrypt.hash(nuevaClave, saltRounds);

        // 3. Actualizar la clave en la base de datos usando el ID del usuario
        await db.query(
            "UPDATE cuentas SET hash = $1 WHERE id = $2",
            [nuevoHash, id]
        );
...
        // 3. Crear el enlace de recuperación (URL de tu frontend)
        // **ESTA ES LA LÍNEA QUE SE CORRIGE: El frontend debe tener una ruta /restablecer-clave**
        const frontendUrl = 'https://metasapp2025-production.up.railway.app';
        const resetUrl = `${frontendUrl}/restablecer-clave?token=${resetToken}`; // <--- ¡CAMBIADO a /restablecer-clave!
...
// Endpoint para refrescar el token (No se modifica)
router.post("/refresh-token", (req, res) => {
    const { refreshToken } = req.body;
    if (!refreshToken) return res.status(403).json({ error: "Refresh token requerido" });
